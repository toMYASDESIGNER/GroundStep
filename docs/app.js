...
function attemptTemplate(caseId, attempt, index) {
  const imageUrl = attempt.evidence?.publicUrl || (attempt.evidence?.filename ? `/evidence/${encodeURIComponent(caseId)}/${encodeURIComponent(attempt.evidence.filename)}` : "");
  const image = imageUrl ? `<img src="${escapeHtml(imageUrl)}" alt="Submitted evidence">` : "";
...
async function load() {
  const staticMode = location.hostname.endsWith("github.io") || new URLSearchParams(location.search).has("static");
  if (staticMode) {
    const data = await fetch("./demo-case.json").then((response) => response.json());
    render(data);
